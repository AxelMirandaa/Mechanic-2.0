...
function traer2() {
    fetch("https://randomuser.me/api/")
        ...
        .then((data) => {
            console.log(data.results["0"]);
            contenido2.innerHTML = `<img src="${data.results["0"].picture.large}" width="100px" class="img-fluid rounded-circle"> 
            <p>${data.results["0"].name.last}</p>
            `;
        });
}
...
$(document).ready(function () {
    $.getJSON("http://127.0.0.1:8000/api/lista_servicios", function (data) {
        $.each(servicio, function (i, item) {
            $("#servicio").append("<p>" + item.nombre_servicio + "</p>");
        });
    });
});
